...
**관측 근거(외부 원천 대조)**: B — r13 세션 ①②④ 스폰 실행 로그 전부 `glm-5.3-Flash` 동일 모델 확인(상관 판정자 실측). C — TESTS.md L315·L334: r10·r11 ②팬아웃 `general-purpose`+`Explore`(§2 테이블 밖) 기록, 위반으로 분류되지 않음 → E5의 ④ 라우팅 위반 검사로 예방(실패 원장화). K2 — r11~r13 claims 전건 grep 채증, r13 실질 결함은 claims 밖 경로(논문 HTML 저자 대조)가 포착.
