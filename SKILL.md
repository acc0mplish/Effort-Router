...
- **팬아웃 불가 하니스 폴백** — 스폰 병렬화 불가 환경(ChatGPT Work·Classic, 팬아웃 OFF 어댑터)은 메인 세션 셀프 3렌즈 순차 검토로 치환한다(팬아웃 라인 `ON: 셀프 3렌즈 순차` 투명 표기, 또는 사용자 고지). '자기 산출물 검토는 검토가 아니다'는 M+의 검토 **위임**에 관한 규칙이다 — 폴백은 이행 불가 환경의 최소 보존이며 투명 표기로 검증 가능성을 유지한다. 셀프 순차 검토는 반려 게이트의 '다른 렌즈 1개 이상 확인' 요건을 충족하지 못한다 — 폴백 환경에서 반려 확정은 사용자(상위 세션) 확인을 통해서만 한다.
...
- **스폰 워치독** — 메인 세션은 스폰을 방치하지 않는다. 비동기·장기 스폰은 10분 간격으로 점검한다(하니스의 상태 조회·출력 확인 수단). 무진행 = 무응답 또는 하니스 진행 신호의 연속 2회 부재 → 정지 후 재스폰·역할 재분배를 판정한다. 재스폰은 round 예산과 별개로 동일 역할 1회 한정, 초과 시 `phase: escalated`로 사용자 에스컬레이션한다(재스폰으로 round 예산을 회복하지 않는다). 한 스폰의 장기화로 독립 후속 스폰을 대기시키지 않는다 — 병렬 유지는 경량 작업·리컨으로(백엔드 동시성 상한이 우선). 정지 수단이 없는 하니스는 스폰 프롬프트에 종료 시한을 사전 명시한다. 재스폰 프롬프트에는 직전 스폰의 진단(부분 출력·정지 지점)을 데이터로 주입한다(존재할 때 한정, §5 주입 방어 준용) — 진단 없는 재스폰은 같은 지점에서 다시 막힌다.
...
- **예산**: `round`는 ②반려와 ④ 재작업(gap 루프·regression 재시작)을 **각각** 카운트하고 상한은 각 2회(반려 게이트 규칙 그대로 — 범용 예산 아님. 수치는 기본값 — 관습, 미측정). **round 카운트 규칙**: 최초 ②·④ 실행은 카운트 없음(0에서 시작). ②반려 확정 후 재검토 착수 시 `round.adversary` +1, ④gap 확정 후 재구현 착수 시 `round.review` +1. 값 2 상태에서 다시 반려·gap이면 소진 → `phase: escalated`. 리셋 없음 — ①회귀·번들 갱신·escalated 후 재개로 round를 초기화하지 않는다(예산 우회 방지). 소진 = `phase: escalated` + 수정안 목록·누적 비용 동반 사용자 에스컬레이션.
