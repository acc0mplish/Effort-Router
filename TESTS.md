...
## r17 외부 하니스 메커니즘 차용 — 무효화 전파·실행 패턴 어휘 (2026-09-19)

revfactory/codex-harness 비교에서 차용 기준 O(1)·일회성 비용 통과 2종을 차용했다. 태스크당·메시지당 세레모니가 붙는 메커니즘(통신 로그 등)은 기존 불채택을 유지한다.

행동 변경 규칙(증류):

| 신규 규칙 | 삽입 위치 | 원전 근거 |
|-----------|-----------|-----------|
| I1 — 무효화 전파 불릿: 번들 원문 수록 의무(미수록 번들 폴백 수록 포함)·트리거 4종·이벤트 구동 1회 감지·트리거별 전파 범위((c)는 claims 갱신=신규 pending·이전 evidence superseded 표기)·round 유지·스키마 필드 0 | SKILL.md §5 '단계 상태 = state.json' 블록, '사용자 개입 반영' 뒤 | codex-harness invalidation propagation |
| I2 — 실행 패턴 어휘 표 6행(전 행 실존 계약 서술 인용) + '신규 메커니즘 아님' 표지 | SKILL.md ●●● 팬아웃 절 말미(심각도 정의 뒤) | codex-harness 실행 패턴 6종 |
| I3 — platforms/CLAUDE.md 두뇌 프로토콜 7단계 무효화 전파 이식 | platforms/CLAUDE.md 두뇌 프로토콜 말미 | platforms/CLAUDE.md 파생 갱신 선언 |
