...
- **대기 시 행동** — 머지·게이트·리뷰·스폰 완료 대기 중 멈춘 채 보고만 기다리지 않는다: 독립 과업(경량 작업 우선)으로 전환, 전환 불가 시 리컨·다음 과업 계획 수립으로 대기를 쓴다(①계획 스폰 대기는 리컨 병렬화). 전환은 대기 예상 시간이 전환 비용보다 길 때다(짧은 대기마다 컨텍스트 전환을 강제하지 않는다). 이미 통과한 게이트·검증의 재실행은 전환 행동에서 제외한다. 어느 대기종이든 한 스폰의 장기화가 독립 후속 스폰을 정지시키지 않는다(병렬 유지는 백엔드 동시성 상한이 우선). 근거: 대기 중 다음 과업 정지·게이트 3회 중복 실행 실측.
...
| 단계 | 역할(HoH¹) | 산출 계약 |
|------|-----------|-----------|
| ①계획 | Project Planner | 수정 대상(우선순위·PR 분할) + 보존 제약 + **검증 요구(claims)** — 산출물 미수정 |
...
- **얇은 계획(깊이는 위험에 비례)**: ①의 최소 완료 기준 = 계약 요소 확정(수정 대상·보존 제약·검증 요구 claims·인터페이스·파일 소유권·PR 분할). **PR 분할**(코드 산출 과업 기준 — 문서 과제 산출 계약은 기존대로) = ①계획이 과업을 PR로 나눠 번들에 포함한다(구현 후 압축 아님): 직렬 의존이 있는 과업은 직렬 의존 스택으로 나눈다(상한 5 PR, 강제 — 직렬 의존 체인 1개 기준), 독립 변경은 병렬 트랙으로, 소규모·독립 과업은 1 PR 허용, 목표 2-3 PR(방향), PR은 Phase(또는 그 묶음)의 머지 단위다. 상한 초과가 필요하면 계획 번들에 근거를 기재한 예외로만 한다. PR별 ④리뷰·머지는 진행을 병행하되 state.json phase는 과업 단위로 유지하며 verify는 최종 PR 머지 후에 수행한다. 구현 중 재분할이 필요하면 메인 판정으로 번들을 갱신한다(① 재작성 루프 아님). PR이 커지면 되돌림 덩어리가 커진다 — 근거: 615 과업에서 6PR 직렬 스택 순차 머지·보고 대기 왕복 다수 실측. 계약 요소가 확정된 스코프는 심층 계획 없이 ③구현 착수를 허용한다 — 잔여 검증은 ④리뷰 claims 대조가 담당한다. 심층 계획은 임계경로·코어 모듈(core-xhigh 대상)과 위험 지표 양성 스코프에만 투자. 깊이는 ① 스폰 프롬프트에 지정한다(에이전트 파일 불변). 본 목적 = 계획 대기 wall-clock 축소.
